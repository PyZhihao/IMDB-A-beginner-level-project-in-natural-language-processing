...
对于单向 `RNN` ，`output`的`shape`为：$\boxed{
...
例如：`h_n[-1]`表示：最后一层 `RNN` 的最后一个时间步的隐藏状态，`shape`为：$\boxed{
[\text{batch_size},\text{hidden_size}]
}$
...
传统`RNN` 在序列较长时，反向传播会经过很多时间步。梯度中会出现连续相乘：$\frac{\partial h_t}{\partial h_{t-1}}
\frac{\partial h_{t-1}}{\partial h_{t-2}}
\cdots
\frac{\partial h_2}{\partial h_1}$，可能会出现两种问题：梯度消失和梯度爆炸
