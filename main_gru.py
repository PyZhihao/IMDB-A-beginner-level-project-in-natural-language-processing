...
def plot_loss(train_losses, val_losses, layers, save_dir):
    ...
    save_path = os.path.join(save_dir, f"loss_curve_gru_{layers}.png")
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        if val_loss < min_val_loss:
            ...
            torch.save(model.state_dict(), f"best_model/best_model_gru_{gru_layers}.pt")

    print(f"Min val loss: {min_val_loss:.4f}")
    print(f"Best val Acc: {best_val_acc:.4f}")

    # 画 loss 曲线
    plot_loss(train_losses, val_losses, gru_layers, "loss_curve")

    # 加载模型参数
    model.load_state_dict(
        torch.load(
            f"best_model/best_model_gru_{gru_layers}.pt",
            map_location=device,
            weights_only=True
        )
    )
