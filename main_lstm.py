...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        if val_loss < min_val_loss:
            ...
            save_checkpoint(model, f"best_model/best_model_lstm_{lstm_layers}.pt")

    print(f"Min val loss: {min_val_loss:.4f}")
    print(f"Best val Acc: {best_val_acc:.4f}")

    # 画 loss 曲线
    plot_loss(train_losses, val_losses, lstm_layers, "loss_curve", "lstm")

    # 加载模型参数
    load_checkpoint(model, f"best_model/best_model_lstm_{lstm_layers}.pt", device)
