...
class RNNClassifier(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, rnn_layers=1, dropout=0.3, pad_idx=0):
        super().__init__()
        # Embedding
    ...
    def forward(self, x, length):
        ...
        # 2. 打包序列, 告诉 RNN 每个 token 的真实长度， 后面的 <pad> 不需要参与计算
...
def plot_loss(train_losses, val_losses, layers, save_dir):
    ...
    save_path = os.path.join(save_dir, f"loss_curve_rnn_{layers}.png")
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        if val_loss < min_val_loss:
            ...
            torch.save(model.state_dict(), f"best_model/best_model_rnn_{rnn_layers}.pt")

    print(f"Min val loss: {min_val_loss:.4f}")
    print(f"Best val Acc: {best_val_acc:.4f}")

    # 画 loss 曲线
    plot_loss(train_losses, val_losses, rnn_layers, "loss_curve")

    # 加载模型参数
    model.load_state_dict(
        torch.load(
            f"best_model/best_model_rnn_{rnn_layers}.pt",
            map_location=device,
            weights_only=True
        )
    )
